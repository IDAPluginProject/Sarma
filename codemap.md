...
| SQLite 持久化 + 迁移框架 | ✅ 完成 | schema v8，结构化 Migration |
...
| Settings Service/Presenter/UI 测试 | ✅ 完成 | 3 个测试文件 |
...
| IDAPython Skill 文档 | ✅ 完成 | `skills/idapython/` (50+ 模块) |
| Router Vuln Audit Skill | ✅ 完成 | `skills/router-fs-vuln-audit/` |
...
- **God Class**：`ChatServiceWorker` 承担过多职责
- **错误处理不完整**：3 个 TODO 错误类未集成
- **Mixin 脆弱性**：`ReasoningChatOpenAIMixin` patch LangChain 内部
- **Magic Strings**：streaming 事件类型应使用 enum
- **审计工作流断层**：定义层 (8 阶段) 与 UI 层 (5 节点) 不一致
...
3. **ChatPage DI 注入**：将服务实例化移出 UI 组件
4. **建立 CI pipeline**：GitHub Actions 自动运行 IDE tests + compile check
5. **Nuitka 打包验证**：产出可用二进制并建立发布流程
